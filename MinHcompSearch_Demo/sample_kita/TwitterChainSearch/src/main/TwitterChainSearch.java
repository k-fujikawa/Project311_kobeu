...
/**
 * Servlet implementation class TwitterChainSearch
 */
@WebServlet("/TwitterChainSearch")
public class TwitterChainSearch extends HttpServlet {
...
//    out.println("<head>");
//    out.println("<title>Test of TwitterChainSearch</title>");
//    out.println("</head>");
//    out.println("<body>");
//
...
//	  out.println("</body>");
   ...
   public void displayHeader(PrintWriter out) {
     ...
     out.println("<a href=\"http://localhost:8080/MinHcompSearch_Demo/TwitterChainSearch\" style=\"color:white;\">ver2.0</a>");
   ...
   public void displayFooter(PrintWriter out) {
     ...
     out.println("<hr>");
     // /MinHcompSearch_Demo/src/main/TwitterChainSearch.java
     out.println("<a href=\"http://localhost:8080/MinHcompSearch_Demo/TwitterChainSearch\">Topに戻る </a>");
     out.println("</div>");
   }
   ...
   public void displayNavbar(PrintWriter out, String queryStr) {
     ...
     out.println("<form action=\"./TwitterChainSearch\" method=\"POST\">");
     out.println("<p>");
...
//     out.println("</span>");
   ...
   public void displayBody(PrintWriter out, String queryStr, List<String> searchList, String reQueryStrFilter) {
     ...
     displayListArea(out, searchList, reQueryStrFilter);
     out.println("</div>");
     displayFooter(out);
     displayJQuery(out);
...
//       out.println("$(\'#secondSearch\').submit();");
    ...
    * @param runQuery
    */
   public void displayListArea(PrintWriter out, List<String> searchList, String reQueryStrFilter) {
   ...
   if (searchList.size() == 1) {
     ...
     if(hitNumber <= tempProfile.size()) {
       firstSearchResult = tempProfile.topTweets(retnum);
     } else {
       firstSearchResult = tempProfile.topTweets(tempProfile.size());
     }
     ...
     if(hitNumber <= tempProfile.size()) {
       ...
       searchResult = tempProfile.topTweets(tempProfile.size());
     }
   }
...
//   List<Map> queryList = getSearchList(searchList);
   ...
   if (searchList.size() != 0 && firstSearchResult.size() != 0){
     ...
     for(Object[] searchArray : firstSearchResult) {
         ...
         out.println("ondblclick=\"var id = $(this).attr('id');$('input:hidden').attr('id',id); $('input:hidden').val('" + searchArray[2] + "'); $('#secondSearch').submit();\"");
...
//           }
   ...
   out.println("<form id=\"secondSearch\" action=\"./TwitterChainSearch\" name=\"secondSearch\" method =\"post\">");
   ...
   if (searchList.size() != 0 && searchResult.size() == 2){
