...
public class RunQuery {
	...
	public RunQuery(String hostname, int port) {
		this.server = hostname + ":" + Integer.toString(port); // 接続するサーバを指定
	}
	...
	public String qs2query(List<String> qs, String field) {
		String qpre = "#combine";
		if (field != null) {
			qpre += "[" + field + "]";
		}
	...
	public List<Map> search1(List<String> qs, int retnum) {
		...
		try {
			...
			for (int i = 0; i < results.length; i++) {
				DBObject tweet_obj = db.getTweet(names[i]); //mongoDBオブジェクト
				String tweet_str = (String) tweet_obj.get("tweet");
				Date date = (Date) tweet_obj.get("date");
	...
	public List<Map> search(String query, int retnum) {
		...
		try {
			env = new QueryEnvironment(); // indri
			if (index != null) { // コンストラクタで指定しなかった場合、nullのはず
				System.out.println("Add index : " + index);
				env.addIndex(index); // インデックスをindriに指定？
			} else if (server != null) {
				System.out.println("Add server : " + server);
				env.addServer(server); // サーバをindriに指定？
			} else {
				System.err.println("You should specify index or server.");
			}
			// String query = qs2query(qs, field);
			System.out.println("QUERY : " + query + "(" + retnum + ")"); //確認
			System.out.println("Index : " + this.index); //確認
			System.out.println("Server : " + this.server); //確認

			ScoredExtentResult[] results = env.runQuery(query, retnum); // 検索結果

			System.out.println("RetNum : " + results.length);

			names = env.documentMetadata(results, "docno"); // results中の素性名"docno"の値一覧String[]を返す
			for (int i = 0; i < results.length; i++) { // docnoを一つづつ取り出して
				DBObject tweet_obj = db.getTweet(names[i]); //mongoDBオブジェクト生成
				String tweet_str = (String) tweet_obj.get("tweet"); //tweet取得？
				Date date = (Date) tweet_obj.get("date"); // data取得？
		...
		return list; // 検索結果tweet一つ一つの情報がマッピングされたマップののリストをかえす
	}
